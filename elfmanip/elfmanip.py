...
class ELFManip(object):
    ...
    def write_proto2(self, outfile):
        ...
        # test adding new sections and segments
        sec = CustomSection(b'/lib64/ld50.so\0', sh_addr = 0x900000)
        seg = CustomSegment(PT_LOAD)
        sec2 = CustomSection(b'content', sh_addr = 0x910000)
        seg2 = CustomSegment(PT_LOAD)
        sec3 = CustomSection(b'surfeit', sh_addr = 0x920000)
        seg3 = CustomSegment(PT_LOAD)
        self.phdrs['max_num'] = self.phdrs['max_num'] + 2 # 42 arbitrary high number
        seg = self.add_segment(seg)
        self.add_section(sec,seg)
        seg2 = self.add_segment(seg2)
        self.add_section(sec2,seg2)
        seg3 = self.add_segment(seg3)
        self.add_section(sec3,seg3)
        ...
        with open(outfile, 'r+b') as f:
            for s in self.elf.iter_sections():
                '''if s.name == '.interp':
                    f.seek(s['sh_offset'] + s['sh_size'])
                    pad_to_modulus(f, 0x10)
                    offset = f.tell() #set offset to aligned address directly after .interp
                    '''
                if s.name == '.text':
                    f.seek(s['sh_offset'])
                    pad_to_modulus(f, 0x10)
                    offset = f.tell() #set offset to aligned address in text

            for p in self.phdrs['entries']:
                if p.p_type == PT_PHDR:
                    p.p_offset = offset
                    p.p_vaddr += (offset - 0x40)
                    p.p_paddr += (offset - 0x40)

            f.seek(0, os.SEEK_END)
            self.ehdr['e_shoff'] = f.tell()

            

            pad_to_modulus(f, PAGESIZE)
            pad_to_modulus(f, PAGESIZE, pad_if_aligned=True) # Shift forwards to add bytes for special section
            for s in self.custom_sections:
                if s.sh_offset is None:
                    # Must be the new section we added TODO: I'm not going to re-implement the code for adding sections,
                    # so this is just a hack to temporarily add one
                    s.sh_offset = f.tell()
                    # interp_phdr.p_offset = s.sh_offset
                    print 'JUST SET OFFSET TO 0x%x and writing %s' % (s.sh_offset, s.contents)
                    f.write(s.contents)
                    pad_to_modulus(f, PAGESIZE)
            self.ehdr['e_phoff'] = offset
            f.seek(0)
    ...
    def write_prototype(self, outfile):
        if outfile == self.filename:
            logger.error("Must specify a different file destination than the original ELF")
            return
        ...
        # test adding a new section and segment
        sec = CustomSection(b'/lib64/ld50.so\0', sh_addr = 0x900000)
        seg = CustomSegment(PT_LOAD)
        sec2 = CustomSection(b'content', sh_addr = 0x910000)
        seg2 = CustomSegment(PT_LOAD)
        sec3 = CustomSection(b'surfeit', sh_addr = 0x920000)
        seg3 = CustomSegment(PT_LOAD)
        self.phdrs['max_num'] = self.phdrs['max_num'] + 3 # 42 arbitrary high number

        #phdr_entry_size = 56
        #phdr_size = len(self.phdrs['entries']) * phdr_entry_size
        offset = 0x1000
        interp_offset = 0
        interp_phdr = None
        with open(outfile, 'w+b') as f:
            ...
            f.write(self.dump_ehdr())
            for p in self.phdrs['entries']:
                if p.p_type == PT_LOAD and p.p_offset == 0:
                    p.p_filesz += offset
                    p.p_memsz += offset
                    p.p_vaddr -= offset
                    p.p_paddr -= offset
                #elif p.p_type == PT_LOAD:
                #    p.p_memsz += offset
                elif p.p_offset > 64:
                    p.p_offset += offset
                    if p.p_type == PT_INTERP:
                        p.p_offset -= offset
                        f.seek(p.p_offset)
                        f.write(b'\0\0\0\0\0') #DESTROY OLD INTERP
                        p.p_offset += 0x100
                        p.p_vaddr -= offset - 0x100
                        p.p_paddr -= offset - 0x100
                        interp_offset = p.p_offset
                        interp_phdr = p
                elif p.p_type == PT_PHDR:
                    p.p_vaddr -= offset
                    p.p_paddr -= offset
                    #p.p_filesz = self.phdrs['max_num']*56 # TODO: this works only for 64-bit
                    #p.p_memsz = self.phdrs['max_num']*56 # TODO: this works only for 64-bit
                '''if p.p_vaddr > offset:
                    p.p_vaddr -= offset
                    p.p_paddr -= offset'''
            seg = self.add_segment(seg)
            ...
            f.seek(offset) # 64 to skip elf, and 56*9 to skip specific phdrs, but let's copy them too....
            with open(self.filename, 'rb') as f2:
                f2.seek(0)
                for x in f2:
                    f.write(x)
                '''b = f2.read(1)
                while b != '':
                    ...
                    f2.read(1)'''
            self.ehdr['e_shoff'] = f.tell()
            # Overwrite interp string with the path to our custom dynamic loader
            f.seek(interp_offset)
            f.write(b'/lib64/ld50.so\0')
            ...
            for s in self.custom_sections:
                if s.sh_offset is None:
                    ...
                    print 'JUST SET OFFSET TO 0x%x and writing %s' % (s.sh_offset, s.contents)
                    f.write(s.contents)
                    pad_to_modulus(f, PAGESIZE) #HAVE TO PAD IT TO AT LEAST A PAGE
            #phdrsec = CustomSection(contents='', name=0x1f, sh_type=SHT_PROGBITS, sh_flags=SHF_ALLOC | SHF_EXECINSTR, sh_addr=0x400040, sh_addralign=0x10, arch='x64')
            #phdrsec.sh_size = phdr_size
            #phdrsec.sh_offset = 0x40 + offset
            #self.add_section(phdrsec)
            dynamic = b''
            dynamic_offs = 0
            for s in self.shdrs['entries']:
                if s.sh_offset > 0:
                    '''if s.sh_offset == 0x238: #Stupid probably nonworking and unneeded special case for interp
                        s.sh_offset += 0x100
                        s.sh_addr += 0x100'''
                    s.sh_offset += offset
                    if s.sh_addr > offset and s.sh_addr < 0x600000:  #TODO: get rid of this hard-coded value
                        s.sh_addr -= offset
                '''if s.sh_type == SHT_DYNAMIC:
                    dynamic_offs = s.sh_offset
                    offs = 0
                    problems = [0xc, 0xd, 0x19, 0x1a, 0x5, \
                          0x6, 0x7, 0x17, 0x6ffffef5,0x6ffffffe,0x6ffffff0] # SYMTAB, 0x3, PLTGOT,  JMPREL, RELA, GNU_HASH, VERNEED, VERSYM
                    while True:
                        # x64-specific
                        entry = struct.unpack_from('<QQ',s.contents,offs)
                        if entry[0] in problems:
                            dynamic += struct.pack('<QQ',entry[0],entry[1]+offset)
                        else:
                            dynamic += struct.pack('<QQ',entry[0],entry[1])
                        if entry[0] == 0: #If it's the last entry
                            break
                        offs += struct.calcsize('<QQ')'''
            f.seek(self.ehdr['e_shoff'])
            f.write(self.dump_shdrs())
            f.seek(64)
            f.write(self.dump_phdrs())
            #f.seek(64 + offset )#put new phdrs at new offset because why not other than tons of reasons but hey gotta try something ELSE
            #f.write(self.dump_phdrs())
            if dynamic_offs != 0:
              f.seek(dynamic_offs)
              f.write(dynamic)
